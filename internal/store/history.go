...
func (s *HistoryStore) GetVersion(ctx context.Context, historyID int64) (*models.HistoryEntry, error) {
	var h models.HistoryEntry
	err := s.db.QueryRow(ctx,
		`SELECT history_id, entry_id, hiveshare_id, user_id, action,
		 ...
		 WHERE history_id = $1`,
		historyID,
	).Scan(&h.HistoryID, &h.EntryID, &h.HiveshareID, &h.UserID, &h.Action,
		&h.Content, &h.Summary, &h.HasEmbedding,
		&h.Tags, &h.Metadata, &h.SourceType, &h.SourceRef, &h.SourceURL, &h.Tool, &h.RecordedAt)
	if err != nil {
...
func (s *HistoryStore) Rollback(ctx context.Context, entryID, hiveshareID uuid.UUID, historyID int64) (*models.MemoryEntry, bool, error) {
...
func (s *HistoryStore) Undelete(ctx context.Context, historyID int64, hiveshareID uuid.UUID) (*models.MemoryEntry, bool, error) {
	var e models.MemoryEntry
	var hasEmbedding bool
	err := s.db.QueryRow(ctx,
		`INSERT INTO memory_entries
		 ...
		 SELECT entry_id, hiveshare_id, user_id, source_type, source_ref, source_url,
		        tool, content, summary, embedding, tags, metadata
		 FROM memory_entries_history
...
func (s *HistoryStore) PurgeByAge(ctx context.Context, olderThan time.Duration) (int64, error) {
	tag, err := s.db.Exec(ctx,
		`DELETE FROM memory_entries_history WHERE recorded_at < NOW() - $1::interval`,
		olderThan.String(),
	)
	if err != nil {
...
func (s *HistoryStore) PurgeByCount(ctx context.Context, maxVersions int) (int64, error) {
	tag, err := s.db.Exec(ctx,
		`DELETE FROM memory_entries_history
		 WHERE history_id IN (
...
func (s *HistoryStore) RestoreSnapshot(ctx context.Context, snapshotID int64, userID uuid.UUID, name string) (*RestoreResult, error) {
	tx, err := s.db.Begin(ctx)
	if err != nil {
		return nil, err
	}
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO hiveshares (name, description, owner_id)
		 ...
		 FROM hiveshare_snapshots s WHERE s.snapshot_id = $3
		 RETURNING id, name, description, owner_id, settings, created_at, updated_at`,
		name, userID, snapshotID,
	...
	rows, err := tx.Query(ctx,
		`SELECT id FROM memory_entries WHERE hiveshare_id = $1 AND embedding IS NULL`,
		hs.ID,
	)
	if err != nil {
...
func (s *HistoryStore) DeleteSnapshot(ctx context.Context, snapshotID int64, hiveshareID uuid.UUID) error {
	_, err := s.db.Exec(ctx,
...
func (s *HistoryStore) CopyEntries(ctx context.Context, targetHiveshareID, userID uuid.UUID, entryIDs []uuid.UUID) ([]*CopyResult, error) {
	...
	for _, eid := range entryIDs {
		...
		err := tx.QueryRow(ctx,
			`SELECT id, source_type, source_ref, source_url, tool,
			        content, summary, embedding, tags, metadata
			 FROM memory_entries WHERE id = $1`,
			eid,
		).Scan(&e.ID, &e.SourceType, &e.SourceRef, &e.SourceURL, &e.Tool,
		...
		var newEntry models.MemoryEntry
		err = tx.QueryRow(ctx,
			`INSERT INTO memory_entries
			     (hiveshare_id, user_id, source_type, source_ref, source_url,
			      tool, content, summary, embedding, tags, metadata)
			...
			targetHiveshareID, userID, e.SourceType, e.SourceRef, e.SourceURL,
