-- Memory entry history: trigger-based audit trail for all content mutations.
-- Enables per-entry rollback/undelete and hiveshare-level snapshots.
...
CREATE INDEX IF NOT EXISTS memory_history_entry_idx
    ON memory_entries_history (entry_id, recorded_at DESC);

CREATE INDEX IF NOT EXISTS memory_history_hiveshare_idx
    ON memory_entries_history (hiveshare_id, recorded_at DESC);

-- ── Trigger function ─────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION record_memory_history() RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS memory_history_trigger ON memory_entries;
CREATE TRIGGER memory_history_trigger
    AFTER INSERT OR UPDATE OF content, summary, tags, metadata, embedding OR DELETE
    ON memory_entries
    FOR EACH ROW EXECUTE FUNCTION record_memory_history();
