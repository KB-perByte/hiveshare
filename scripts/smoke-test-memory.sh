...
CREATE_CODE=$(curl -s -o $SMOKE_TMPDIR/mem_create.json -w "%{http_code}" -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory" \
    -H "$AUTH_A" -H "Content-Type: application/json" \
...
BAD_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory" \
    -H "$AUTH_A" -H "Content-Type: application/json" -d '{"content":"no source"}')
smoke_check "$BAD_CODE" "400" "missing fields returns 400"

VIEW_WRITE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory" \
...
smoke_section "List"
LIST_CODE=$(curl -s -o $SMOKE_TMPDIR/mem_list.json -w "%{http_code}" "$SMOKE_BASE/hiveshares/$HS_ID/memory" -H "$AUTH_A")
...
FILTERED=$(curl -sf "$SMOKE_BASE/hiveshares/$HS_ID/memory?source_type=jira" -H "$AUTH_A")
...
smoke_section "Get"
GET_CODE=$(curl -s -o $SMOKE_TMPDIR/mem_get.json -w "%{http_code}" "$SMOKE_BASE/hiveshares/$HS_ID/memory/$ENTRY_ID" -H "$AUTH_A")
...
SEARCH_CODE=$(curl -s -o $SMOKE_TMPDIR/mem_search.json -w "%{http_code}" -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory/search" \
    -H "$AUTH_A" -H "Content-Type: application/json" \
...
SEARCH_400=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory/search" \
...
UPD_CODE=$(curl -s -o $SMOKE_TMPDIR/mem_upd.json -w "%{http_code}" -X PUT "$SMOKE_BASE/hiveshares/$HS_ID/memory/$ENTRY_ID" \
    -H "$AUTH_A" -H "Content-Type: application/json" \
...
DEL_ENTRY=$(curl -sf -X POST "$SMOKE_BASE/hiveshares/$HS_ID/memory" \
    -H "$AUTH_A" -H "Content-Type: application/json" \
    -d '{"source_type":"manual","source_ref":"del","content":"delete me","tool":"manual","tags":[]}')
DEL_ID=$(echo "$DEL_ENTRY" | jq -r '.id')
DEL_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE \
    "$SMOKE_BASE/hiveshares/$HS_ID/memory/$DEL_ID" -H "$AUTH_A")
